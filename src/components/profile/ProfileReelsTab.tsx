  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
          ...
          {videosInRepo.length === 0 && uploadingItems.length === 0 ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs text-slate-500 px-1">
                <span>
                  {videosInRepo.length} vídeo{videosInRepo.length !== 1 ? "s" : ""} disponível{videosInRepo.length !== 1 ? "is" : ""}
      ...
      <Modal
        ...
        title={previewVideo?.name || "Visualizar Vídeo"}
        description={
          previewVideo
            ? `Vídeo de @${account.username} • ${formatBytes(previewVideo.sizeBytes)}${previewVideo.durationSeconds ? ` • ${formatDuration(previewVideo.durationSeconds)}` : ""}`
            : undefined
        }
        maxWidth="md"
      >
        {previewVideo && (
          <div className="flex flex-col items-center justify-center gap-4">
            <div className="relative aspect-[9/16] w-full max-w-[280px] bg-black rounded-2xl overflow-hidden shadow-xl border border-slate-800">
              <video
            ...
            <div className="w-full bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs text-slate-600 flex justify-between items-center">
              <span>
                Status: <strong className="text-emerald-700 font-semibold uppercase">{previewVideo.retentionStatus || "ativo"}</strong>
              </span>
              <span>Enviado em: {formatDate(previewVideo.createdAt)}</span>
            </div>
          </div>
        )}
      </Modal>
