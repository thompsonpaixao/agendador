...
export function calculateScheduleSlots({
  ...
  while (scheduledSlots.length < itemsCount && daysIterated < maxDays) {
    ...
    for (const timeStr of sortedTimes) {
      ...
      // Aplica variação aleatória de horário apenas UMA vez no momento da criação
      if (useRandomVariation && randomVariationMinutes > 0) {
        const deltaMinutes =
          Math.floor(Math.random() * (2 * randomVariationMinutes + 1)) - randomVariationMinutes;
        scheduledMs += deltaMinutes * 60 * 1000;
      }

      // Se o horário calculado para hoje já passou (com margem de 60s), avança para o próximo slot válido
      if (scheduledMs <= nowMs + 60000) {
        continue;
      }
