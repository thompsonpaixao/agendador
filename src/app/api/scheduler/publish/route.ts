  ...
  try {
    ...
    if (!usedRpc) {
      ...
      const { data: scheduledCandidates, error: scheduledErr } = await supabaseAdmin
        ...
        .select("id, scheduled_at, instagram_account_id, post_type, queue_id, status, locked_at, meta_container_id, next_retry_at")
        .eq("status", "scheduled")
      ...
      if (allCandidates.length > 0) {
        ...
        // Filtra candidatos válidos (não bloqueados recentemente, retry vencido e fila não pausada)
        const validCandidates = allCandidates.filter((post: any) => {
          if (post.queue_id && pausedQueueSet.has(post.queue_id)) {
            return false;
          }
          // Se o post tiver um retry agendado no futuro (~5 min), aguarda o momento correto
          if (post.next_retry_at) {
            const retryTime = new Date(post.next_retry_at).getTime();
