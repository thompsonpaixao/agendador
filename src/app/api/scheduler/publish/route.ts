...
export async function POST(request: Request) {
  try {
    ...
    console.log(`[Scheduler Publish] Executado em ${nowIso}. Verificando posts agendados...`);

    // 3. Tenta claim atômico via RPC (PostgreSQL FOR UPDATE SKIP LOCKED)
    let claimedPosts: Array<{ id: string; scheduled_at: string; instagram_account_id: string; post_type: string; queue_id?: string | null }> = [];
    let usedRpc = false;
    ...
    // 4. Fallback resiliente: consulta direta caso RPC não esteja disponível ou retorne vazio
    if (!usedRpc) {
      const { data: candidates, error: fetchError } = await supabaseAdmin
        .from("scheduled_posts")
        .select("id, scheduled_at, instagram_account_id, post_type, queue_id, status, locked_at")
        .eq("status", "scheduled")
      ...
      if (fetchError) {
        console.error("[Scheduler Publish] Erro ao buscar candidatos a publicação:", fetchError);
        return NextResponse.json(
      ...
      if (candidates && candidates.length > 0) {
        ...
        const validCandidates = candidates.filter((post) => {
          if (post.queue_id && pausedQueueSet.has(post.queue_id)) {
            return false;
          }
          if (post.locked_at) {
            const lockedTime = new Date(post.locked_at).getTime();
            if (now.getTime() - lockedTime < 10 * 60 * 1000) {
              return false; // Bloqueado por outro worker ativo
            }
          }
          return true;
        });
    ...
    // 5. Tratamento de Posts Vencidos sem Processamento (Item 3)
    // Se algum post venceu há mais de 15 minutos e não está sendo processado por nenhum worker,
    // registra aviso técnico para evitar que passe despercebido.
    ...
    if (claimedPosts.length === 0) {
      return NextResponse.json({
        success: true,
        message: "Nenhum post agendado pendente para este minuto.",
        due: 0,
        claimed: 0,
        processed: 0,
        published: 0,
