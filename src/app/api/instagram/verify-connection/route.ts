  ...
  try {
    ...
    if (supabaseAdmin) {
      ...
      if (!secretError && secret) {
        // Se token_auth_tag estiver ausente em um registro antigo, avisa especificamente para reconectar
    ...
    const graphVersion = metaGraphVersion.startsWith("v") ? metaGraphVersion : `v${metaGraphVersion}`;

    try {
      const metaResponse = await fetch(
        `https://graph.instagram.com/${graphVersion}/me?fields=id,username,account_type&access_token=${rawToken}`
      );
