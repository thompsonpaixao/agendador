...
export async function POST(request: Request) {
  try {
    ...
    const { accountId, filename, hasThumbnail } = body;
    ...
    const videoFileName = `${uniquePrefix}_${cleanName}`;
    const thumbFileName = `${uniquePrefix}_thumb.jpg`;

    // Organização estrita de pastas:
    // users/{user_id}/instagram/{instagram_account_id}/reels/{arquivo}
    const videoStoragePath = `users/${user.id}/instagram/${accountId}/reels/${videoFileName}`;
    const thumbStoragePath = `users/${user.id}/instagram/${accountId}/reels/thumbnails/${thumbFileName}`;

    // Gera Signed Upload URL para o vídeo
    const { data: videoUpload, error: videoUploadError } = await supabaseAdmin.storage
      .from("media")
      .createSignedUploadUrl(videoStoragePath, { upsert: true });

    if (videoUploadError || !videoUpload) {
      console.error("[Upload URL] Erro ao gerar URL para vídeo:", videoUploadError);
      return NextResponse.json(
        { success: false, message: `Falha ao preparar upload no storage: ${videoUploadError?.message}` },
        { status: 500 }
      );
    }
