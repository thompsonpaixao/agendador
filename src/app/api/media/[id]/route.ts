  ...
  try {
    ...
    if (futurePosts && futurePosts.length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: "Esta mídia está agendada para publicação futura e não pode ser excluída.",
        },
        { status: 400 }
      );
    }
    ...
    if (activeQueueItems && activeQueueItems.length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: "Esta mídia faz parte de uma fila ativa de Reels e não pode ser excluída.",
        },
        { status: 400 }
      );
    }
