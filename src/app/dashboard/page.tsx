...
export default function DashboardPage() {
  ...
  // Publicados Hoje calculado estritamente no fuso horário America/Sao_Paulo
  const todaySP = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Sao_Paulo" }).format(new Date());
  const postsPublishedToday = filteredPublished.filter((p) => {
    if (!p.publishedAt) return false;
    const postDateSP = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Sao_Paulo" }).format(new Date(p.publishedAt));
    return postDateSP === todaySP;
  });
  const postsToday = postsPublishedToday.length;
  ...
  // Correção do bug do "13": Conta estritamente os que estão com status "scheduled"
  const actualScheduledList = filteredScheduled.filter((p) => p.status === "scheduled");
  const postsScheduled = actualScheduledList.length;
