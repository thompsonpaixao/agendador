...
export default function DashboardPage() {
  ...
  const accountsWithRate = filteredAccounts.filter((a) => a.successRate != null);
  const successRateText =
    publishedPosts.length > 0 && accountsWithRate.length > 0
      ? `${(
          accountsWithRate.reduce((acc, a) => acc + (a.successRate ?? 0), 0) /
          accountsWithRate.length
        )
          .toFixed(1)
          .replace(".", ",")}%`
      : "—";
